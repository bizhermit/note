...
※ カスケードマージを連鎖させるとGitHubActionが無限ループ防止機能が働くため、回避するにはトークンに[PAT](https://docs.github.com/ja/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens)を使用する必要がある  
※ GitHubの設定でsupportブランチへの直プッシュ禁止（PR必須）にすると、自動化スクリプトも止められるため除外対応が必要になる  
...
→ ブランチ名に存在しないバージョンを指定すればチェックスクリプトで誤マージを防ぐことは可能  
...
- supportブランチ間で後続supportへマージする作業の難易度が高い
  - → CIツールで自動化する（ただしコンフリクトが発生する可能性有り → 自動でPRが作成される）
