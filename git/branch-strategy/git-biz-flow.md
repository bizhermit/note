...
特定のmainブランチが存在しないため、最新バージョンのsupportブランチが実質main/stagingブランチ。  
...
インフラに制約があっても何とかするための検証用の掃きだめ。もしくはＱＡチームへの提供用。  
...
4. testで検証完了後、featureをsupportでリベース、PRを作成する
5. supportへのPRをマージする
6. 後続supportが存在する場合は、基点supportから後続supportへマージする
7. supportからreleaseにマージしてリリース
8. releaseに対する修正は、support経由でhotfixを作成し行う
